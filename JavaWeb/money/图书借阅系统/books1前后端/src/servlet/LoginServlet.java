        ...
        try {
            ...
            if (usernameList.contains(loginName)) {
                ...
                if (passwordList.get(0).getPassword().equals(loginPassword)) {
                    ...
                    // 根据用户类型跳转到不同页面
                    if (passwordList.get(0).getIsadmin() == 1) {
                        // 管理员跳转到管理页面
                        response.sendRedirect("/admin/MangeIndex.jsp" + queryString);
                    } else {
                        // 普通用户直接跳转到BorrowBookServlet以显示图书列表
                        response.sendRedirect("/BorrowBookServlet" + queryString);
                    }
                } else {
